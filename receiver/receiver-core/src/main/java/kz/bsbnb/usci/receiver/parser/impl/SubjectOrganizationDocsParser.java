    ...
    public boolean startElement(XMLEvent event, StartElement startElement, String localName) throws SAXException {
        switch (localName) {
            case "docs":
                break;
            case "doc":
                BaseEntity organizationDoc = new BaseEntity(refDocTypeMeta, respondentId, batch.getReportDate(), batch.getId());
                organizationDoc.put("code",  new BaseValue(event.asStartElement().getAttributeByName(new QName("ref_doc_type")).getValue()));
                currentBaseEntity.put("ref_doc_type", new BaseValue(organizationDoc));
                break;
            case "name":
